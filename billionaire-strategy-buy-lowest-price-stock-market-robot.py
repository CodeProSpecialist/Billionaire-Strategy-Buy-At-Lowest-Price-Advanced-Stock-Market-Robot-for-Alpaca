...
# quick, small gains. See ProfitMonitorEngine._thresholds_for().
ATR_ARM_MULTIPLIER = 0.30        # arm at max(ARM_PROFIT_PCT, 0.30 x ATR%)
ATR_GIVEBACK_FRACTION = 0.20     # giveback = 20% of the gain achieved at peak
...
def _in_moo_sweep_window(now):
    """
    True from MOO_SWEEP_HOUR:MOO_SWEEP_MINUTE through the next 4 minutes
    Eastern. The wait loop ticks every 60s, so an exact single-minute match
    can be missed if one iteration runs slow; a few minutes of tolerance
    still submits comfortably before the 9:30 open while the once-per-day
    guard in run_premarket_profit_sweep() ensures it only fires once.
    """
    return _in_time_window(now, MOO_SWEEP_HOUR, MOO_SWEEP_MINUTE)
...
def buy_stocks(symbols_to_buy_list, lock):
    ...
    for symbol in list(symbols_to_buy_list):
        ...
        try:
            ...
            # expected reward / expected risk: score is the reward proxy, ATR% the
            # risk proxy. Guard against a zero/near-zero ATR% blowing up the ratio.
            rank_score = sig['score'] / max(atr_pct, 0.001) if atr_pct else sig['score']
    ...
    if to_buy:
        ...
        for i, c in enumerate(to_buy, 1):
            ...
            print(f"  #{i} {c['yf_symbol']}: score {c['sig']['score']}, "
                  f"ATR% {atr_pct_str}, rank {c['rank_score']:.1f}")
...
class ProfitMonitorEngine:
    ...
    @staticmethod
    def _thresholds_for(atr_pct):
        """
        REVIEW ITEM #2: arm/giveback scale with the stock's own volatility
        instead of using the same flat percentages for a quiet utility and a
        wild small-cap. Falls back to the flat constants when ATR% is
        unavailable (e.g. a fresh symbol with no ATR history yet).
        """
        if atr_pct is None or atr_pct <= 0:
            return ARM_PROFIT_PCT, PEAK_GIVEBACK_PCT
        arm = max(ARM_PROFIT_PCT, ATR_ARM_MULTIPLIER * atr_pct)
        giveback = max(PEAK_GIVEBACK_PCT, ATR_GIVEBACK_FRACTION * arm)
        return arm, giveback
    ...
    def evaluate(self, symbol, entry_price, current_price, atr_pct=None):
        ...
        arm_pct, giveback_pct = self._thresholds_for(atr_pct)
            ...
            if USE_SCALED_EXITS and SCALE_OUT_STAGES:
                ...
                for idx, (trigger_pct, frac) in enumerate(SCALE_OUT_STAGES):
                    ...
                    print(f"{symbol}: {GREEN}+{gain_now*100:.2f}%{RESET} hit scale-out stage "
                          ...
                          f"({frac*100:.0f}% of original), moving floor to breakeven.")
                    try:
                        so = api.submit_order(symbol=symbol, qty=str(scale_qty), side='sell',
                                              type='market', time_in_force='day')
                        logging.info(f"{now_str} Scale-out stage {idx+1} for {symbol}: "
                                    f"submitted sell {scale_qty:.4f} sh at ~{current_price:.2f}.")
                    except Exception as e:
                        print(f"{symbol}: scale-out sell failed: {e}")
                        logging.error(f"{symbol}: scale-out sell failed: {e}")
                        break
                    _scale_out_mark_fired(symbol, idx)
                    ...
                    with open(csv_filename, mode='a', newline='') as f:
                        csv.DictWriter(f, fieldnames=fieldnames).writerow({
                            ...
                            'Quantity': scale_qty, 'Symbol': symbol,
