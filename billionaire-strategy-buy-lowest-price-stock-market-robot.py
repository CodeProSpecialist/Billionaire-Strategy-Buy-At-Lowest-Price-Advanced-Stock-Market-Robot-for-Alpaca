...
# reads electricity-or-utility-stocks-to-buy-list.txt at run time.
...
def _backtest_get_universe():
    if BACKTEST_SYMBOLS_SOURCE == 'candidate_list':
        try:
            with open('electricity-or-utility-stocks-to-buy-list.txt') as f:
...
def buy_stocks(symbols_to_buy_list, lock):
    ...
    for symbol in list(symbols_to_buy_list):
        ...
        try:
            ...
            # distance uses the intraday mean of 1-min samples as the volume-
            # unweighted VWAP proxy (true VWAP would require per-tick volume;
            # this is the best approximation from data we have on hand).
            ...
            if len(intraday_samples) >= 5:
                mean_intraday = sum(intraday_samples) / len(intraday_samples)
                if mean_intraday > 0:
                    vwap_distance_pct = (mean_intraday - current_price) / mean_intraday
