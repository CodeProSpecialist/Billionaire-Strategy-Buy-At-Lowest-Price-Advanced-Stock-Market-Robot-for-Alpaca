...
# ---------------- Exit strategy ----------------
# The trailing stop and the +0.5% take-profit are two competing exits on the
# same shares. A GTC trailing stop RESERVES shares at the broker, so a later
# take-profit sell can only touch the unreserved fraction unless the stop is
# cancelled first (see cancel_open_sell_orders).
# Note a 1% trailing stop will usually trigger before a +0.5% target is reached
# and held, so with both enabled the stop is the de-facto exit. Set this False
# to let the take-profit run the exits cleanly.
USE_TRAILING_STOP = True
TRAIL_PERCENT = 1.0
TAKE_PROFIT_PCT = 1.005          # +0.5% over entry
...
def sell_stocks(symbols_to_sell_dict, lock):
    ...
    for symbol, (bought_price, purchase_date) in list(symbols_to_sell_dict.items()):
        ...
        try:
            ...
            logging.info(f"{now_str} Submitted sell {sell_qty} sh of {symbol} at ~{current_price:.2f}")
    ...
    while True:
        try:
            ...
            buy_thread = threading.Thread(target=_run_and_release,
                                          args=(buy_stocks, symbols_to_sell_dict, symbols_to_buy, lock))
            sell_thread = threading.Thread(target=_run_and_release,
                                           args=(sell_stocks, symbols_to_sell_dict, lock))
            buy_thread.start()
            sell_thread.start()
            buy_thread.join()
            sell_thread.join()
