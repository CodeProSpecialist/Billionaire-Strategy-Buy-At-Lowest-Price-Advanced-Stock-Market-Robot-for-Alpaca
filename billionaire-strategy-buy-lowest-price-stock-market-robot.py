...
def run_stock_scanner():
    ...
    try:
        import pandas as _pd  # ensure imported (pandas is already imported below via yfinance chain)
    except Exception:
        pass
    ...
    scores = []
    for years_ago in _SCANNER_CONFIG['lookback_years']:
...
def _do_scanner_refresh():
    ...
    if new_list:
        ...
        print(f"[scanner] SYMBOLS_TO_BUY_LIST refreshed: {len(new_list)} symbols")
    else:
...
def refresh_symbols_to_buy_list_if_due():
    ...
    if cache_empty:
        print("[scanner] cache is empty; running initial scan synchronously...")
        _do_scanner_refresh()
        return
